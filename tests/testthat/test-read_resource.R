...
test_that("read_resource() can read inline data (ignoring schema)", {
  p <- example_package()
  expected_df <- readr::read_csv(
    test_path("data/media.csv"),
    col_types = "ccccc"
  )
  expect_identical(read_resource(p, "media"), expected_df)
...
test_that("read_resource() can read safe local and remote schema, including
  ...
  expect_identical(read_resource(p_local_schema, "deployments"), expected_df)
  ...
  expect_identical(read_resource(p_remote_schema, "deployments"), expected_df)

  # Schema is YAML
  p_yaml_schema <- p
  p_yaml_schema$resource[[1]]$schema <- test_path("data/deployment_schema.yaml")
  expect_identical(read_resource(p_yaml_schema, "deployments"), expected_df)
})
...
test_that("read_resource() can read safe local and remote CSV dialect", {
  ...
  expect_identical(read_resource(p_local_dialect, "deployments"), expected_df)
  ...
  expect_identical(read_resource(p_remote_dialect, "deployments"), expected_df)

  # Dialect is YAML
  p_yaml_dialect <- p
  p_yaml_dialect$resource[[1]]$dialect <- test_path("data/dialect.yaml")
  expect_identical(read_resource(p_yaml_dialect, "deployments"), expected_df)
})
...
test_that("read_resource() understands CSV dialect", {
  ...
  expect_identical(resource_dialect, expected_df)
})
...
test_that("read_resource() understands missing values", {
  ...
  expect_identical(read_resource(p_missing, "deployments"), expected_df)
})

test_that("read_resource() understands undefined missing values", {
  p <- example_package()
  expected_df <- read_resource(p, "deployments") # Only contains default "" in data
  p$resources[[1]]$schema$missingValues <- NULL
  expect_identical(read_resource(p, "deployments"), expected_df)
})
...
test_that("read_resource() understands encoding", {
  ...
  expect_identical(read_resource(p_encoding, "deployments"), expected_df)
...
test_that("read_resource() handles decimalChar/groupChar properties", {
  ...
  df <- read_resource(p, "mark_num_default")
  expect_identical(df$num, expected_num_value) # 3000000.30
  expect_identical(df$num_undefined, expected_num_value) # 3000000.30
  ...
  df <- suppressWarnings(read_resource(p, "mark_num_decimal"))
  expect_identical(df$num, expected_num_value) # 3000000.30
  expect_identical(df$num_undefined, expected_num_value) # 3000000.30
  ...
  df <- suppressWarnings(read_resource(p, "mark_num_decimal_group"))
  expect_identical(df$num, expected_num_value) # 3.000.000,30
  ...
  expect_identical(df$num_undefined_group_invalid, NA_real_) # 3.000.000,30
  ...
  df <- suppressWarnings(read_resource(p, "mark_int_group"))
  expect_identical(df$int, expected_int_value) # 3,000,000
  expect_identical(df$int_undefined, expected_int_value) # 3000000
  ...
  df <- suppressWarnings(read_resource(p, "mark_int_group_num_decimal"))
  expect_identical(df$int, expected_int_value) # 3.000.000
  expect_identical(df$num, expected_num_value) # 3000000,30
})
...
test_that("read_resource() handles LF and CRLF line terminator characters", {
  ...
  p <- example_package()
  expected_df <- read_resource(p, "deployments") # This file has LF

  p_crlf <- p
  attr(p_crlf, "directory") <- "."
  p_crlf$resources[[1]]$path <-
    test_path("data/deployments_crlf.csv") # File with CRLF
  expect_identical(read_resource(p_crlf, "deployments"), expected_df)
})
...
test_that("read_resource() can read compressed files", {
  ...
  expect_identical(read_resource(p_local_zip, "deployments"), expected_df)
  expect_identical(read_resource(p_remote_zip, "deployments"), expected_df)
  expect_identical(read_resource(p_local_gz, "deployments"), expected_df)
  expect_identical(read_resource(p_remote_gz, "deployments"), expected_df)
})

## Field types ----
test_that("read_resource() handles strings", {
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "string")
  expect_type(df$str, "character")

  # Use factor when enum is present
  enum <- unlist(p$resources[[1]]$schema$fields[[2]]$constraints$enum)
  expect_s3_class(df$str_factor, "factor")
  expect_identical(levels(df$str_factor), enum)
})

test_that("read_resource() handles numbers", {
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "number")

  # Leading/trailing zeros are optional, + is assumed
  expect_type(df$num, "double")
  expect_true(all(df$num == 3))
  expect_type(df$num_neg, "double")
  expect_true(all(df$num_neg == -3))

  # Use factor when enum is present
  enum <- p$resources[[2]]$schema$fields[[3]]$constraints$enum
  expect_s3_class(df$num_factor, "factor")
  expect_identical(levels(df$num_factor), as.character(enum))

  # NaN, INF, -INF are supported, case-insensitive
  expect_type(df$num_nan, "double")
  expect_true(all(is.nan(df$num_nan)))
  expect_type(df$num_inf, "double")
  expect_true(all(df$num_inf == Inf))
  expect_type(df$num_ninf, "double")
  expect_true(all(df$num_ninf == -Inf))

  # Number can be expressed with E+-digits
  expect_type(df$num_sci, "double")

  # bareNumber = false allows whitespace and non-numeric characters
  expect_type(df$num_ws, "double")
  expect_true(all(df$num_ws == 3.1))
  expect_type(df$num_notbare, "double")
  expect_true(all(df$num_notbare == 3.1))
})

test_that("read_resource() handles integers (as doubles)", {
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "integer")

  # Leading/trailing zeros are optional, + is assumed
  expect_type(df$int, "double")
  expect_true(all(df$int == 3))
  expect_type(df$int_neg, "double")
  expect_true(all(df$int_neg == -3))

  # Use factor when enum is present
  enum <- p$resources[[3]]$schema$fields[[3]]$constraints$enum
  expect_s3_class(df$int_factor, "factor")
  expect_identical(levels(df$int_factor), as.character(enum))

  # bareNumber = false allows whitespace and non-numeric characters
  expect_type(df$int_ws, "double")
  expect_true(all(df$int_ws == 3))
  expect_type(df$int_notbare, "double")
  expect_true(all(df$int_notbare == 3))
})

test_that("read_resource() handles booleans", {
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "boolean")

  # Default trueValues/falseValues are cast to logical
  expect_type(df$bool_true, "logical")
  expect_type(df$bool_false, "logical")
  expect_true(all(df$bool_true == TRUE))
  expect_true(all(df$bool_false == FALSE))
})

test_that("read_resource() handles dates", {
  expected_value <- as.Date("2013-11-23")
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "date")
  # This test covers:
  # - year: %Y %y
  ...
  expect_identical(df$dt_undefined, df$dt_default)
  expect_identical(df$dt_undefined, expected_value)
  expect_identical(df$dt_default, expected_value)
  expect_identical(df$dt_any, expected_value)
  expect_identical(df$dt_shortcut, expected_value)
  expect_identical(df$dt_1, expected_value)
  expect_identical(df$dt_2, expected_value)
  expect_identical(df$dt_3, expected_value)
})

test_that("read_resource() handles times", {
  expected_value <- hms::hms(0, 30, 8) # "08:30:00"
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "time")
  # This test covers:
  ...
  expect_identical(df$tm_undefined, df$tm_default)
  expect_identical(df$tm_undefined, expected_value)
  expect_identical(df$tm_default, expected_value)
  expect_identical(df$tm_any, expected_value)
  expect_identical(df$tm_shortcut, expected_value)
  expect_identical(df$tm_1, expected_value)
  expect_identical(df$tm_2, expected_value)
  expect_identical(df$tm_3, hms::hms(0.3, 30, 8)) # "08:30:00.3"
...
test_that("read_resource() handles datetimes", {
  ...
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "datetime")

  expect_identical(df$dttm_undefined, expected_value)
  expect_identical(df$dttm_default, expected_value)
  expect_identical(df$dttm_any, expected_value)
  expect_identical(df$dttm_1, expected_value)
  expect_identical(df$dttm_1_default, expected_value)
  expect_identical(df$dttm_2, expected_value_ms)
  expect_identical(df$dttm_2_default, expected_value_ms)
})

test_that("read_resource() handles other types", {
  p <- read_package(test_path("data/types.json"))
  df <- read_resource(p, "other")

  # Interpret year, yearmonth as dates
  expect_s3_class(df$year, "Date")
  expect_identical(df$year[1], as.Date("2001-01-01"))
  expect_s3_class(df$yearmonth, "Date")
  expect_identical(df$yearmonth[1], as.Date("2001-03-01"))

  # Interpret object, array, list, duration, geopoint, geojson as character
  expect_type(df$object, "character")
  expect_type(df$array, "character")
  expect_type(df$list, "character")
  expect_type(df$duration, "character")
  expect_type(df$geopoint, "character")
  expect_type(df$geojson, "character")

  # Interpret any as character
  expect_type(df$any, "character")

  # Interpret undefined as character
  # Note: unknown values are blocked by check_schema()
  expect_type(df$no_type, "character")
})
