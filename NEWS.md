...
* `cli::cli_abort()`, `cli::cli_warn()` and `cli::cli_inform()` are used for all errors, warnings, and messages (#163). This has several advantages:
  ...
  * Each call has an [rlang](https://cran.r-project.org/package=cli) class, e.g. `frictionless_error_fields_without_name`, making it easier to test for specific errors.
* The dependencies [glue](https://cran.r-project.org/package=cli) and [assertthat](https://cran.r-project.org/package=cli) are removed (#163). The functionality of glue is replaced by cli, `assertthat::assert()` calls are now `if()` functions.
