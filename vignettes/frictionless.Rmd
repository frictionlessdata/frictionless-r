...
Your Data Package now contains 2 resources, but you can add metadata properties as well (see the [Data Package specification](https://datapackage.org/standard/data-package/#properties) for an overview). Let's add `name` as first and `title` as second property:
...
my_package <- add_properties(
  ...
  name = "my_package",
  title = "My package",
  after = 0 # Add as the first and second property
)
```
