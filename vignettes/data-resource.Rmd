...
You can also remove any of those resources with `remove_resource()`:
...
When creating a resource, `add_resource()` also only supports Tabular Data Resources. The provided data must therefore be a data frame or a tabular file (e.g. CSV):

```{r}
# Add (tabular) resource from a data frame (but don't assign to x)
add_resource(x, "iris", data = iris)

# Add (tabular) resource from a file (but don't assign to x)
