...
`resource()` gets a resource from a package by its name. The returned object is a list you can [manipulate](#manipulate):
...
::: {.callout-important}
Note that `resource()` and `resource()<-` are designed for internal use in other packages. For public manipulation of resources, use `add_resource()` and `remove_resource()`.
:::
...
`add_properties()` can be used to add (custom) metadata properties to a resource. This can also be done with `add_resource()`, except for properties automatically set by that function.
...
resource <- add_properties(
  ...
  after = 0 # Add as the first property
)
str(resource)

# Overwrite the resource in the package with the updated one
resource(package, "deployments") <- resource
```
...
- `read_resource()` ignores it and does not upgrade a resource, since it does not rely on v1 properties deprecated in v2.
