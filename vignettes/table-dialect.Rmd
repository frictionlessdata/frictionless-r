...
frictionless supports dialect to correctly interpret [Tabular Data Resources](https://specs.frictionlessdata.io/tabular-data-resource/).

### Read

`read_resource()` uses the `dialect` property of a resource (see `vignette("data-resource")`) to correctly interpret a tabular data file. Only properties that deviate from the default need to be specified. E.g. a tab-delimited file without header rows should have the following dialect:
...
frictionless does not support direct manipulation of the dialect. Its properties are set when writing.

### Write

When creating a resource from a file, `add_resource()` only sets one dialect property: `delimiter` and only if it deviates from the default `","`.

When creating a resource from a data frame, `add_resource()` does not set any dialect properties. `write_package()` writes this data frame to a CSV file, but does not set any dialect properties either, since only the defaults are used.

## Properties

The sections below describe how frictionless supports dialect properties.
...
When creating a resource from a file, `add_resource()` sets `delimiter` based on the provided `delim` (e.g. `","` (default), `"\t"`, etc.).
