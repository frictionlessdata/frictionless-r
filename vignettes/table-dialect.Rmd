...
frictionless supports dialect to parse [Tabular Data Resources](https://specs.frictionlessdata.io/tabular-data-resource/). `dialect` can either be a JSON object or a path or URL referencing a JSON object.
...
`write_package()` writes a package to disk as a `datapackage.json` file. This file will include the metadata of all the resources, including the dialect (if defined). `write_resource()` writes resources created from a data frame to CSV files, but no `dialect` property is set for those, since only defaults are used.

## Dialect properties

### delimiter

[`delimiter`](https://specs.frictionlessdata.io/csv-dialect/#specification) is supported by `read_resource()` and defaults to `","`. It is passed to `delim` in `readr::read_delim()`. `add_resource()` does not set `delimiter`, unless provided in `delim` and different from the default `","`:
...
[`lineTerminator`](https://specs.frictionlessdata.io/csv-dialect/#specification) is ignored by `read_resource()`. It relies instead on `readr::read_delim()`, which interprets line terminator `LF` and `CRLF` automatically and does not support `CR` (used by Classic Mac OS, final release 2001).
...
[`nullSequence`](https://specs.frictionlessdata.io/csv-dialect/#specification) is ignored by `read_resource()`. Provide instead as `missingValues` in the schema (see `vignette("table-schema")`).
